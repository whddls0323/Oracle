...
insert into "order" values(5,'c105',3,1,'2023-01-01 13:15:18');

--실습 6-3
select orderNo,name,prodName,orderCount,orderDate from customer c,product p,"order" o where c.custid=o.orderId and p.prodNo=o.orderProduct;
select orderNo,orderProduct,prodName,price,orderCount,orderDate from customer c,product p,"order" o where p.prodNo=o.orderProduct and orderId=(select custId from customer where name='김유신');
select sum(price * orderCount) from product p,"order" o where p.prodNo=o.orderProduct;